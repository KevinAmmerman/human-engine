...
export function createThreads({ cfg, stateDir, socialMemory, observedStore, log }) {
  ...
  function collectOpenTopics(scope, agentId) {
    ...
    for (const p of Object.values(people)) {
      ...
      for (const t of threads) {
        ...
        const entryTs = typeof t.lastTs === "number" ? t.lastTs : personLastTs;
        if (entryTs > 0 && now - entryTs > topicExpiryMs) continue;
        const topic = String(t.topic || "").trim();
        if (!topic) continue;
        const who = String(t.whoOwesWhat || "").toLowerCase();
        const awaiting = ownNames.length > 0 && who.length > 0 && ownNames.some((n) => who.includes(n))
          ? "agent"
          : "member";
        topics.push({
          topic,
  ...
  return { onActivity, onSpeak, contextFor, stop, __stateForTests: () => ({ cache, dirty, flushTimers }) };
}
