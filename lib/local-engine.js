...
export function hasHardTrigger(prompt, messages, agentName, agentContactIds) {
  ...
  const name = (agentName || "OpenClaw").toLowerCase();
...
export function createLocalEngine({ cfg, llm, timing, log }) {
  ...
  async function decide({ sessionKey, messages, systemPrompt, isDM, hasMedia, transcript, persona, voiceCard, agentName, prompt, agentContactIds }) {
    ...
    if (hasHardTrigger(prompt, messages, agentName, agentContactIds)) {
    ...
    try {
      ...
      if (text === "SPEAK") return { decision: "speak", epoch, path: "llm" };
      return { decision: "stay_silent", epoch, path: "llm" };
    } catch (err) {
