...
export function createLocalEngine({ cfg, llm, timing, log }) {
  ...
  async function decide({ sessionKey, messages, systemPrompt, isDM, hasMedia, mediaKind, transcript, persona, voiceCard, agentName, prompt, agentContactIds, replyToAgent, agentAliases, memoryContext, threadContext }) {
    ...
    const decidePrompt = buildDecidePrompt({ transcript, persona, voiceCard, agentName, mediaKind, memoryContext, threadContext, v2Contract: v2, language: cfg?.language });
  ...
  async function respond({ sessionKey, draft, epoch, systemPrompt, agentName, transcript, persona, voiceCard, isGroup, replyTarget, triggerInfo, language }) {
    ...
    const splitPrompt = buildSplitPrompt({ draft, transcript, persona, voiceCard, maxBubbles, replyTarget, language });
