...
export function createLocalEngine({ cfg, llm, timing, log }) {
  ...
  async function decide({ sessionKey, messages, systemPrompt, isDM, hasMedia, mediaKind, transcript, persona, voiceCard, agentName, prompt, agentContactIds, replyToAgent, agentAliases, memoryContext }) {
    ...
    const decidePrompt = buildDecidePrompt({ transcript, persona, voiceCard, agentName, mediaKind, memoryContext, v2Contract: v2 });
  ...
  function estimateReadMs(triggerInfo, transcript) {
    ...
    const mediaMarkerRe = /\[(image|video|audio|document|sticker|voice message)\]/i;
    const lines = Array.isArray(transcript) ? transcript : [];
    const hasMedia = lines.some((l) => mediaMarkerRe.test(String(l?.text || "")));
    if (hasMedia) {
      ms += 1500;
    }
    return ms;
  ...
  async function respond({ sessionKey, draft, epoch, systemPrompt, agentName, transcript, persona, voiceCard, isGroup, replyTarget, triggerInfo }) {
    ...
    const splitPrompt = buildSplitPrompt({ draft, transcript, persona, voiceCard, maxBubbles, replyTarget });
  ...
  return { decide, respond, currentEpoch: (sk) => epochs.get(sk) || 0, extractVoiceCard, enhancePersona, regenerateReply };
}
