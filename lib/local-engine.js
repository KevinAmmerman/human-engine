import { buildDecidePrompt, buildSplitPrompt, buildExtractPrompt, renderPromptBlock, buildEnhancePrompt } from "./local-prompts.js";
...
function hasHardTrigger(prompt, messages, agentName) {
  const text = (prompt || "").toLowerCase();
  const allText = [
    text,
    ...(messages || []).map((m) => (m.text || m.content || "").toLowerCase()),
  ].join(" ");
  if (allText.includes((agentName || "OpenClaw").toLowerCase())) return true;
  return false;
}
...
export function createLocalEngine({ cfg, llm, timing, log }) {
  ...
  async function decide({ sessionKey, messages, systemPrompt, isDM, hasMedia, transcript, persona, voiceCard, agentName, prompt }) {
    const epoch = (epochs.get(sessionKey) || 0) + 1;
    epochs.set(sessionKey, epoch);
    capMap();

    if (isDM || hasMedia) {
      return { decision: "speak", epoch };
    }

    if (hasHardTrigger(prompt, messages, agentName)) {
      return { decision: "speak", epoch };
    }
    ...
    try {
      ...
      if (text === "SPEAK") return { decision: "speak", epoch };
      return { decision: "stay_silent", epoch };
    } catch (err) {
      _log.warn("local-engine: decide LLM error: %s", err?.message || err);
      return null;
    }
  }
  ...
  async function respond({ sessionKey, draft, epoch, systemPrompt, agentName, transcript, persona, voiceCard, isGroup }) {
    ...
    try {
      ...
      _log.warn("local-engine: respond LLM error: %s", err?.message || err);
      return { scheduled: [{ content: draft, position: 0, delayMs: 100 }], superseded: false };
    }
  }
  ...
  return { openThread, decide, respond, currentEpoch: (sk) => epochs.get(sk) || 0, extractVoiceCard, enhancePersona };
}
