...
/**
 * Runtime backstop (plan 027): strip mechanical tells (fail-open) so a
 * regressing split/humanize model never lands em-dashes, markdown or list
 * markers in the group. Semantic tells (banned words, its-not-its,
 * summary-closing, enumeration) are DETECTED and counted but NOT replaced —
 * string surgery on meaning is worse than the tell, so they are logged only.
 * Never throws; non-string/empty input passes through unchanged.
 */
...
// Plan 615: guard the split LLM — every non-empty bubble must be a verbatim
// substring of the draft (normalized). Catches pronoun flips, added reactions,
// and invented sentences while tolerating punctuation/casing splits. Empty /
// emoji-only bubbles normalize to "" and are ignored. Pure; never throws.
export function isFaithfulSplit(draft, bubbles) {
  try {
    const draftNorm = normalizeFaithful(String(draft ?? ""));
    if (!draftNorm) return true;
    for (const b of bubbles || []) {
      const raw = typeof b === "string" ? b : (b && typeof b.content === "string" ? b.content : "");
      const bn = normalizeFaithful(raw);
      if (bn && !draftNorm.includes(bn)) return false;
    }
    return true;
  } catch {
    return true;
