...
const GERMAN_STARTER =
  /^(?:Per|Und|Das|Ich|Du|Wir|Also|Ja|Nee|Haha|Okay|Ok|Nicht|Ein|Eine|Der|Die|Den|Wer|Was|Wo|Wann|Warum|Wie|Hori)\b|^\p{Extended_Pictographic}/iu;
...
export function stripMetaCommentary(text, memberNames = []) {
  if (typeof text !== "string" || !text.trim()) return { text, stripped: false };
  ...
  if (
    ...
    if (keptStart < paras.length) {
      ...
      if (isAcceptableKept(kept, memberNames)) return { text: kept, stripped: true };
    }
  }
