...
export function findAgentContactIds(map, agentName, agentAliases = []) {
  const ids = new Set();
  if (!map || !agentName) return ids;
  const needles = [String(agentName), ...(agentAliases || [])].map((n) => String(n).toLowerCase());
  for (const [id, name] of map.entries()) {
    const raw = String(name);
    const bare = raw.replace(/\s*\([^)]*\)\s*$/, "").trim().toLowerCase();
    const lower = raw.toLowerCase();
    if (needles.some((needle) => lower.startsWith(needle) || bare.startsWith(needle))) ids.add(id);
  }
  return ids;
}
