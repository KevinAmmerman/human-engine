...
export function createGate({ cfg, engine, persona, socialMemory, observedStore, readTranscript, log, proactive }) {
  ...
  function onMessageReceived(event, ctx) {
    ...
    _log.info(`human-engine: message_received fired sk=${sk} isGroup=${ctx?.isGroup} agent=${agentId}`);
    try {
      if (!sk) return;
      ...
      if (replyToSender || replyToBody) {
        ...
        _log.info(`human-engine: reply-context captured sk=${sk} sender=${String(replyToSender || "").slice(0, 40)} bodyLen=${replyToBody.length}`);
      }
  ...
  async function onBeforeAgentReply(event, ctx) {
    ...
    try {
      ...
      _log.info(`human-engine: reply-resolve sk=${sk} sender=${String(replyCtxSender).slice(0, 40)} quotedName=${quotedName || "-"} replyToAgent=${replyToAgent}`);
      ...
      try {
        ...
        try {
          ...
          _log.warn(`human-engine: fail-closed resolveSender error sk=${sk}: ${innerErr?.message || innerErr}`);
        }
        if (sk && isGroup(sk) && prompt && !isCommand(prompt)) {
          return markStaySilent(sk, senderName, prompt, -1, "gate-error-fail-closed");
        }
      } catch (innerErr) {
        _log.warn(`human-engine: fail-closed recovery error sk=${ctx?.sessionKey}: ${innerErr?.message || innerErr}`);
      }
      return;
    }
  }
