...
export function createGate({ cfg, engine, persona, socialMemory, observedStore, readTranscript, log, proactive }) {
  ...
  function onMessageReceived(event, ctx) {
    ...
    try {
      ...
      if (replyToSender || replyToBody) {
        state.replyContextBySession.set(sk, { sender: replyToSender || "", body: replyToBody, ts: Date.now() });
        capMap(state.replyContextBySession, 4096);
      } else {
        state.replyContextBySession.delete(sk);
      }
  ...
  async function onBeforeAgentReply(event, ctx) {
    ...
    try {
      ...
      const replyCtx = state.replyContextBySession.get(sk);
      let replyToAgent = false;
      if (replyCtx && Date.now() - replyCtx.ts < 5 * 60 * 1000) {
        const quotedName = resolveContactName(loadContacts(cfg.contactsPath), replyCtx.sender);
        if (quotedName && cfg.agentName && quotedName.toLowerCase().startsWith(cfg.agentName.toLowerCase())) {
          ...
          replyToAgent = peekArr.some((l) => l.startsWith(ownPrefix) && head.length >= 8 && l.includes(head));
        }
      }
