...
export function createGate({ cfg, state: st, engine, persona, socialMemory, persistObserved, readTranscript, log }) {
  ...
  async function resolveTranscript(sk, ctx, senderName, prompt) {
    ...
    if (peek.length >= 6) return peek;
    if (typeof readTranscript !== "function") return peek;
    try {
      const hydrated = await readTranscript(sk, ctx?.sessionId, 20);
      if (Array.isArray(hydrated) && hydrated.length > 0) {
        const last = hydrated[hydrated.length - 1];
        if (!last || !String(last.text || "").endsWith(prompt.slice(0, 80))) {
          hydrated.push({ speaker: senderName, text: prompt });
        }
        return hydrated.slice(-20);
      }
    } catch {
    }
    return peek;
  ...
  function onMessageReceived(event, ctx) {
    ...
    try {
      ...
      const senderName = resolveSender(ctx?.senderName, ctx?.senderId);
  ...
  async function onBeforeAgentReply(event, ctx) {
    ...
    try {
      ...
      _log.info(`human-engine: claim sk=${sk} isDM=${isDM} decision=${String(decision)} epoch=${epoch}`);
