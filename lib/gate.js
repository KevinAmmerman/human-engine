...
import { loadContacts, resolveContactName } from "./contacts.js";
...
export function createGate({ cfg, state: st, engine, persona, socialMemory, persistObserved, log }) {
  ...
  function onMessageReceived(event, ctx) {
    ...
    try {
      ...
      if (socialMemory && isEnabled(cfg) && isScopedAgent(cfg, ctx?.agentId) && isChatSession(sk)) {
        const scope = (ctx.agentId || "?") + "::" + sk;
        socialMemory.ingest(scope, { speaker: resolveSender(ctx?.senderName, ctx?.senderId), text, ts: Date.now() });
      }
    } catch {
    }
  }
  ...
  async function onBeforeAgentRun(event, ctx) {
    ...
    try {
      ...
      const senderName = resolveSender(ctx?.senderName, ctx?.senderId, event?.senderName, event?.senderId);

      const peekArr = state.transcriptPeekBySession.get(sk);
      const alreadyPeeked = peekArr && peekArr.length > 0 && peekArr[peekArr.length - 1].endsWith("] " + prompt);
      if (!alreadyPeeked) {
      ...
      if (decision === "stay_silent") {
        ...
        if (epoch != null) {
          state.silentEpochBySession.set(sk, { epoch, ts: Date.now() });
      ...
      if (engineResult === null) {
        if (isGroup(sk)) {
          pushObserved(sk, `[${senderName}] ${prompt}`);
          persist(sk, senderName, prompt);
          state.silentEpochBySession.set(sk, { epoch: epoch ?? -1, ts: Date.now() });
          capMap(state.silentEpochBySession, 4096);
          _log.info(`human-engine: group fail-closed marked sk=${sk} epoch=${epoch ?? -1} (suppress at reply)`);
        }
        return;
      }
