...
import { toServiceMessages, isCommand, splitQuotedAudioBody } from "./messages.js";
...
export function createGate({ cfg, engine, persona, socialMemory, observedStore, readTranscript, log, proactive, onSilence, threads, mood }) {
  ...
  async function onBeforeAgentReply(event, ctx) {
    ...
    try {
      ...
      const { userText, quotedTranscript } = splitQuotedAudioBody(prompt);
      const modelPrompt = quotedTranscript
