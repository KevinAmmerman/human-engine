...
export function createGate({ cfg, state: st, engine, persona, socialMemory, persistObserved, readTranscript, log }) {
  ...
  function mergeTranscriptLayers(hydrated, peek, current) {
    ...
    if (current && current.text && !tailIsIncluded(current)) {
      merged.push({ speaker: current.speaker || "", text: current.text });
    }
  ...
  async function resolveTranscript(sk, ctx, senderName, prompt) {
    ...
    return mergeTranscriptLayers(hydrated, peek, { speaker: senderName, text: prompt });
  }
