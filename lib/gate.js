...
import { toServiceMessages, isCommand, splitQuotedAudioBody, normalizeQuotedAudioBody } from "./messages.js";
...
export function createGate({ cfg, engine, persona, socialMemory, observedStore, readTranscript, log, proactive, onSilence, threads, mood }) {
  ...
  function mergeTranscriptLayers(hydrated, ...layers) {
    // Plan 621 rev: the host can fold a quoted voice transcript into the body.
    // The observed store / hydrated LCM copies keep it raw while the model-facing
    // peek keeps the labelled form. Canonicalize every entry to the labelled
    // form so the raw and labelled copies of the same member message collapse in
    // dedup (and the model sees only the labelled line). Persistence stays raw.
    const canonText = (entry) => normalizeQuotedAudioBody(typeof entry === "string" ? entry : entry?.text);
    const canonEntry = (entry) => ({ speaker: entry.speaker || "", text: canonText(entry), ts: entry.ts });
    const merged = Array.isArray(hydrated)
      ? hydrated.map((e) => (e && typeof e.text === "string" ? canonEntry(e) : e))
      : [];
    ...
    const tailIsIncluded = (line) => {
      const tail = canonText(line).slice(-200);
  ...
  async function resolveTranscript(sk, ctx, senderName, prompt, media) {
    ...
    if (media?.hasMedia) {
      ...
      current.text = (media.marker || "[media]") + (body ? " " + body : "");
    }
  ...
  function markSpeak(sk, agentId, senderName, epoch, agentName, involved, path, inboundText) {
    if (inboundText) persist(sk, senderName, inboundText);
    if (epoch != null) {
      state.speakEpochBySession.set(sk, { epoch, ts: Date.now() });
      capMap(state.speakEpochBySession, 4096);
    }
    if (path) {
      state.speakPathBySession.set(sk, path);
      capMap(state.speakPathBySession, 4096);
    }
    ...
    if (isChatSession(sk)) threads?.onSpeak?.(sk, agentId);
  }
  ...
  function onMessageReceived(event, ctx) {
    ...
    try {
      ...
      if (replyToSender || replyToBody) {
        ...
        // message (ctx.messageId), not the message the member quoted — quoting
        // the quoted message makes the agent appear to reply to its own message.
        // `quotedId` is still used below for reply detection (quotedName/
        // replyToAgent).
        const quotedId = ctx?.replyToId || event?.replyToId || "";
        const msgId = ctx?.messageId || event?.messageId || quotedId || "";
        q.push({ sender: replyToSender || "", body: replyToBody, textNorm: normText(text), msgId, ts: Date.now() });
      ...
      if (isEnabled(cfg) && isScopedAgent(cfg, agentId)) {
        ...
        if (isChatSession(sk)) threads?.onActivity?.(sk, agentId);
  ...
  async function onBeforeAgentReply(event, ctx) {
    ...
    try {
      ...
      // Plan 621: the host may fold a quoted voice transcript into the current
      // body ("[Audio]\nUser text:\n<U>\nTranscript:\n<T>"). Label the quoted
      // part as context so it cannot masquerade as the current ask. The raw body
      // stays the value persisted to the observed store.
      const { quotedTranscript } = splitQuotedAudioBody(prompt);
      const modelPrompt = normalizeQuotedAudioBody(prompt);
      ...
      if (displayText) pushPeekDedup(sk, senderName, quotedTranscript ? modelPrompt : displayText, typeof event?.timestamp === "number" ? event.timestamp : Date.now());
      ...
      const decidePersona = persona?.buildPersonaPrompt
        ...
        : null;
      const transcriptLines = await resolveTranscript(sk, ctx, senderName, modelPrompt, cachedMedia);
      ...
      const threadCtx = threads?.contextFor
        ? threads.contextFor(sk, ctx?.agentId || agentIdFromSessionKey(sk) || null) || null
        : null;
      const threadActive = threadCtx != null;
      ...
      if (socialMemory && isChatSession(sk)) {
        ...
        const compact = socialMemory.recallCompact(scope, involved, 400, { omitOpenThreads: threadActive });
      ...
      const persistReplyTarget = () => {
        if (quotedName || replyToAgent) {
          state.replyTargetBySession.set(sk, {
            ...
            // replyToId = the message id of the reply's quote target. Captured from the
            // reply-context entry (plan 035): the quoted-message id (ctx.replyToId) when the
            // inbound is a quote-reply, else the inbound message's own id (ctx.messageId).
            // Only persisted when the target is real (quotedName OR replyToAgent), same gate
            // as the rest of the target. Null when no id was derivable.
            replyToId: replyCtx && replyCtx.msgId ? replyCtx.msgId : null,
            ts: Date.now(),
          });
      ...
      const applyVerdict = (shared) => {
        ...
        _log.info(`human-engine: claim sk=${redactSessionKey(sk)} isDM=${isDM} decision=${String(decision)} path=${shared.path} epoch=${epoch}${shared.decideReason ? ` reason=${shared.decideReason}` : ""}${shared.addressedTo ? ` addressed=${shared.addressedTo}` : ""}`);
        if (decision === "speak") {
          markSpeak(sk, ctx?.agentId, senderName, epoch, agentIdentity.name, involved, shared.path, displayText);
          persistReplyTarget(sk);
          return;
        }
        if (decision === "stay_silent") {
          return markStaySilent(sk, senderName, displayText, epoch, shared.reason || "decide");
        }
        if (shared.engineNull) {
          if (isGroup(sk)) {
            return markStaySilent(sk, senderName, displayText, epoch ?? -1, "group-fail-closed");
          }
          return;
        }
        return;
      ...
      if (!isTriggerMsg && inflight) {
        ...
        if (shared.decision === "speak") {
          markSpeak(sk, ctx?.agentId, senderName, shared.epoch, agentIdentity.name, involved, shared.path, displayText);
          persistReplyTarget(sk);
          return;
        }
        if (shared.decision === "stay_silent") {
          return markStaySilent(sk, senderName, displayText, shared.epoch, "decide-dedup");
        }
        if (shared.engineNull) {
          if (isGroup(sk)) return markStaySilent(sk, senderName, displayText, -1, "group-fail-closed-dedup");
          return;
        }
        return;
      }

      const moodEnergy = isGroup(sk) && mood?.snapshotFor ? (mood.snapshotFor(ctx?.agentId, sk)?.energy ?? null) : null;
      ...
      const decidePromise = (async () => {
        if (isGroup(sk)) {
          ...
          let selfVoiceLen = 0;
          try {
            const activeSelfVoice = persona?.snapshotFor ? persona.snapshotFor(agentIdFromSessionKey(sk)) : null;
            selfVoiceLen = typeof activeSelfVoice === "string" ? activeSelfVoice.length : 0;
          } catch {}
          _log.info(`human-engine: decide-ctx sk=${redactSessionKey(sk)} lines=${ctxLines.length} own=${ownCount} lastSpeaker=${lastSpeaker} lastAgeMs=${lastAgeMs} selfVoiceLen=${selfVoiceLen}`);
        }
        const engineResult = await engine.decide({
          ...
          voiceCard: null, // card travels inside the persona block — no duplication
          agentContactIds,
          replyToAgent,
          memoryContext,
        ...
        return {
          ...
          decideReason: engineResult?.reason,
          addressedTo: engineResult?.addressedTo,
        };
      })();
      if (!isTriggerMsg) {
      ...
      try {
        ...
        const cachedMedia = sk ? state.mediaBySession.get(sk) || null : null;
        const displayText = prompt || (cachedMedia?.marker ? cachedMedia.marker : "");
        let senderName = "User";
        try {
        ...
        if (sk && isGroup(sk) && displayText && !isCommand(displayText)) {
          return markStaySilent(sk, senderName, displayText, -1, "gate-error-fail-closed");
        }
      } catch (innerErr) {
  ...
  function onBeforePromptBuild(event, ctx) {
    try {
      ...
      // Plan 035: opt-in, groups-only, model-routed reaction capability hint. The plugin
      // NEVER sends reactions itself (no SDK surface); this only tells the model it MAY use
      // its own message tool's react action. Off by default (reactions.hintEnabled:false).
      if (cfg?.reactions?.hintEnabled === true && isGroup(sk)) {
        const hint =
