...
export function createGate({ cfg, state: st, engine, persona, socialMemory, persistObserved, log }) {
  ...
  async function onInboundClaim(event, ctx) {
    _log.info(`human-engine: inbound_claim fired sk=${ctx?.sessionKey || event?.sessionKey} agent=${ctx?.agentId} isGroup=${ctx?.isGroup} len=${(event?.content || "").length}`);
    try {
      if (!isEnabled(cfg)) return;
      if (!isScopedAgent(cfg, ctx?.agentId)) return;

      const prompt = event?.content || event?.body || "";
      if (!prompt) return;
  ...
  return { onMessageReceived, onInboundClaim, onBeforePromptBuild };
}
