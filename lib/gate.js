...
export function createGate({ cfg, engine, persona, socialMemory, observedStore, readTranscript, log, proactive, onSilence, threads, mood }) {
  ...
  function markSpeak(sk, agentId, senderName, epoch, agentName, involved, path) {
    if (epoch != null) {
  ...
  function onMessageReceived(event, ctx) {
    ...
    try {
      ...
      if (replyToSender || replyToBody) {
        ...
        // msgId = the QUOTED message's id (ctx.replyToId — the reply's quote target)
        // when this inbound IS a quote-reply; otherwise the inbound's own message id
        // (ctx.messageId) as the fallback target when we reply to the current message.
        // Priority per plan 035: quoted-message id wins over the inbound's own id.
  ...
  async function onBeforeAgentReply(event, ctx) {
    ...
    try {
      ...
      const applyVerdict = (shared) => {
        ...
        if (decision === "speak") {
          markSpeak(sk, ctx?.agentId, senderName, epoch, agentIdentity.name, involved, shared.path);
      ...
      if (!isTriggerMsg && inflight) {
        ...
        if (shared.decision === "speak") {
          markSpeak(sk, ctx?.agentId, senderName, shared.epoch, agentIdentity.name, involved, shared.path);
          persistReplyTarget(sk);
          return;
        }
