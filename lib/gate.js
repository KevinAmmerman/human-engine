...
export function createGate({ cfg, engine, persona, socialMemory, observedStore, readTranscript, log, proactive, onSilence }) {
  ...
  function markSpeak(sk, agentId, senderName, epoch, agentName) {
    if (epoch != null) {
  ...
  async function onBeforeAgentReply(event, ctx) {
    ...
    try {
      ...
      const systemPrompt = persona?.buildPersonaPrompt(agentCfg, sk);
      ...
      const applyVerdict = (shared) => {
        ...
        if (decision === "speak") {
          markSpeak(sk, ctx?.agentId, senderName, epoch, agentIdentity.name);
      ...
      if (!isTriggerMsg && inflight) {
        ...
        if (shared.decision === "speak") {
          markSpeak(sk, ctx?.agentId, senderName, shared.epoch, agentIdentity.name);
          persistReplyTarget(sk);
          return;
        }
