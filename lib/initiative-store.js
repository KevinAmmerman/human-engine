...
export function createInitiativeStore({ stateDir, log }) {
  ...
  // Plan 614: enumerate the per-scope state files for one agent. Fail-safe:
  // bad/unreadable files are skipped; returns parsed states carrying `scope`.
  function listScopesForAgent(agentId) {
    const base = path.join(stateDir || ".", "initiative", pathSafe(agentId || "?"));
    let names = [];
    try {
    ...
    for (const d of names) {
      ...
      try {
        const parsed = JSON.parse(fs.readFileSync(path.join(base, d.name), "utf8"));
        if (parsed && typeof parsed === "object" && typeof parsed?.scope === "string" && parsed.scope) {
          out.push(parsed);
        }
      } catch {}
    }
  ...
  // Plan 617: topic ledger accessors. findByTopicKey reads the persisted
  // per-scope state; listOpenTasksForAgent aggregates open tasks across all
  // scope files for one agent (each row carries its owning `scope`).
