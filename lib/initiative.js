...
export function createInitiative({ cfg, stateDir, log, llm, runtime, state, threads, now, rng, outbox, store: injectedStore }) {
  const _log = log || { info() {}, warn() {}, debug() {} };
  const _now = typeof now === "function" ? now : () => Date.now();
  const _rng = typeof rng === "function" ? rng : () => Math.random();
  // Plan 619: accept a shared initiative store so dm-proactive writes to the
  // SAME ledger instance (coherent cache). Absent → own store, byte-identical.
  const store = injectedStore || createInitiativeStore({ stateDir, log });
  ...
  // Plan 617: date-independent topic identity. Unlike taskId (which folds in
  // createdAt), the same topic re-extracted later maps to the same key, so a
  // durable ledger can dedupe by topic.
  ...
  function mergeExtract(state, parsed, scope, lastSpeaker, lastMessageTs, now) {
    ...
    // Plan 617: index existing tasks by stable topic key so a re-extraction of
    // the same topic merges into the existing record (refresh lastUpdateTs)
    // instead of creating a new one.
    const byTopic = new Map();
    for (const t of state.tasks) {
      if (t.topicKey && !byTopic.has(t.topicKey)) byTopic.set(t.topicKey, t);
    }
    ...
    for (const raw of Array.isArray(parsed.tasks) ? parsed.tasks : []) {
      ...
      const topicKey = topicKeyOf(normalize(truncated));
      const existingTopic = byTopic.get(topicKey);
      if (existingTopic) {
        existingTopic.lastUpdateTs = createdAt;
        continue;
      }
  ...
  function scrollCandidates(stateObj, now, scope) {
    const ini = cfg?.initiative || {};
    const everyMs = (ini.everyMinutes ?? 60) * 60000;
    const firstNudgeMs = (ini.firstNudgeMinutes ?? 120) * 60000;
    // Plan 617: expire stale open tasks so they stop surfacing. Persist once
    // if anything changed (the no-candidate path below returns without saving).
    ...
    if (expired && scope) store.save(scope, stateObj);
  ...
  async function tickScope(scope, now) {
    ...
    const candidates = scrollCandidates(stateObj, now, scope);
    ...
    if (ini.shadow === true) {
      // Plan 617: mirror the live accounting so the gate sees real counters
      // during shadow and the shadow KPI log reflects what live would do.
    ...
    try {
      ...
      // Plan 617: record the per-task cooldown so the gate read at the top of
      // this function actually fires on the next tick.
  ...
  function adminAddTask(sk, agentId, text) {
    try {
      ...
      const task = {
        id,
        topicKey: topicKeyOf(normalize(clean)),
        kind: "task",
        text: clean,
        people: [],
        createdAt: now,
