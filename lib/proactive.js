...
import { buildProactiveDecidePrompt } from "./local-prompts.js";
...
const MESSAGE_TEMPLATES = {
  unanswered_question: (c) =>
    `A group member asked a question that is still unanswered and you can answer it. The question: "${short(c.anchor)}". Write one short, helpful reply in the group's voice.`,
  stalled_exchange: (c) =>
  ...
  context_match: (c) =>
    `You hold a fact relevant to the current topic: "${short(c.anchor)}". If sharing it genuinely helps the group, send one short message.`,
  follow_up_commitment: (c) =>
    `You promised the group something earlier: "${short(c.anchor)}". Deliver the outcome or a brief status update now in one short message.`,
};
...
function overlapCount(text, facts) {
