...
const CELEBRATION_WORD_RE = /\b(geschafft|bestanden|erledigt|personal best|pb|onsight|flash)\b/i;
const CELEBRATION_SIGNAL_RE = /[!！🎉🥳💪🔥🙌👏🎊]|\bendlich\b/i;
...
export function createProactive({ cfg, state, engine, socialMemory, observedStore, runtime, stateDir, log, now }) {
  ...
  async function onInbound(sk, opts = {}) {
    try {
      ...
      if (pcfg?.triggers?.outcomeCelebration !== false && text) {
        if (CELEBRATION_WORD_RE.test(text) && CELEBRATION_SIGNAL_RE.test(text)) {
      ...
      if (pcfg?.triggers?.checkInOnPromise !== false) {
        const peek = readTranscriptPeek(sk, 10);
        const agentInWindow = peek.some((l) => l.speaker === agentName);
        const memberPromise = peek.find(
          (l) => l.speaker !== agentName && PROMISE_RE.test(String(l.text || "")),
        );
