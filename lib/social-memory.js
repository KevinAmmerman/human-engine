...
function isSelfName(cfg, name) {
  const n = String(name || "").trim().toLowerCase();
  if (!n) return false;
  const needles = [String(cfg?.agentName || ""), ...(cfg?.agentAliases || [])]
...
export function createSocialMemory({ cfg, llm, stateDir, log }) {
  ...
  function getOrLoadProfile(scope) {
    ...
    for (const name of Object.keys(profile.people)) {
      if (isSelfName(cfg, name)) continue;
  ...
  function ingest(scope, { speaker, text, ts }) {
    try {
      ...
      if (isSelfName(cfg, speaker)) return;
  ...
  function recall(scope, involvedNames) {
    try {
      ...
      const names = Object.keys(people).filter((n) => !isSelfName(cfg, n));
