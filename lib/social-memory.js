...
export function createSocialMemory({ cfg, llm, stateDir, log }) {
  ...
  async function extract(scope) {
    try {
      ...
      const scopeAgentId = parseScope(scope)?.agentId;
      const prompt = schemaV2
        ? buildMemoryExtractPromptV2({
            existingProfile: JSON.stringify(existingProfile),
            newMessages,
            agentName: resolveAgentConfig(cfg, scopeAgentId)?.agentName,
          })
      ...
      if (parsed && parsed.people && typeof parsed.people === "object") {
        ...
        let v2Extracted = 0;
        let v2AnyField = false;
        for (const [name, data] of Object.entries(parsed.people)) {
          if (!data || typeof data !== "object") continue;
          if (isSelfName(cfg, name, scopeAgentId)) continue;
          ...
          v2Extracted++;
          if (schemaV2) {
            const merged = mergedPeople[name];
            const hasThreads = Array.isArray(merged.open_threads) && merged.open_threads.length > 0;
            const hasRel = typeof merged.relationship === "string" && merged.relationship.trim() !== "";
            const hasEmo = typeof merged.emotional_state === "string" && merged.emotional_state.trim() !== "";
            if (hasThreads || hasRel || hasEmo) v2AnyField = true;
          }
        }
        if (schemaV2 && v2Extracted > 0 && !v2AnyField) {
          _log.warn(`human-engine: memory-v2 fields empty agent=${scopeAgentId} people=${v2Extracted} — check buildMemoryExtractPromptV2`);
        }
  ...
  function selectPeople(scope, involvedNames) {
    const profile = getOrLoadProfile(scope);
    ...
    for (const name of names) {
      ...
      if (isExact || isPrefix) {
        selected.push({ name, p: people[name] });
      } else {
        remaining.push({ name, p: people[name] });
      }
    }

    remaining.sort((a, b) => (b.p.lastSeenTs || 0) - (a.p.lastSeenTs || 0));
    return selected.concat(remaining.slice(0, 3));
  }

  function renderPerson(entry, schemaV2, factsCap, prefsCap, threadsCap) {
    const { name, p } = entry;
    const facts = p.facts || [];
    ...
    if (!schemaV2) {
      const itemParts = [];
      if (facts.length > 0) itemParts.push(facts.slice(0, factsCap).join(", "));
      if (prefs.length > 0) itemParts.push("prefers " + prefs.slice(0, prefsCap).join(", "));
      if (sit) itemParts.push(sit);
      if (itemParts.length === 0) return null;
      return name + ": " + itemParts.join("; ");
    }
    const rel = p.relationship ? " relationship: " + p.relationship : "";
    const th = (p.open_threads || []).slice(0, threadsCap)
    ...
    if (prefs.length > 0) itemParts.push("prefers " + prefs.slice(0, prefsCap).join(", "));
    if (sit) itemParts.push(sit);
    if (rel) itemParts.push(rel);
    if (th.length > 0) itemParts.push(th.join(""));
    if (emo) itemParts.push(emo);
    if (itemParts.length === 0) return null;
    return name + ": " + itemParts.join("; ");
  }

  function capResult(result, limit) {
  ...
  function recall(scope, involvedNames) {
    try {
      if (!isEnabled()) return "";
      const parts = [];
      const caps = schemaV2 ? [2, 1, 2] : [3, 2, 2];
      for (const entry of selectPeople(scope, involvedNames)) {
        const line = renderPerson(entry, schemaV2, caps[0], caps[1], caps[2]);
        if (line) parts.push(line);
      }
      return capResult(parts.join(". "), recallLimit);
    } catch {
      return "";
    }
  }

  function recallCompact(scope, involvedNames, limitChars = 400, options) {
    try {
      if (!isEnabled()) return "";
      const omitThreads = options?.omitOpenThreads === true;
      const parts = [];
      for (const entry of selectPeople(scope, involvedNames)) {
        if (omitThreads) {
          const line = renderPerson(entry, schemaV2, 1, 1, 0);
          if (line) parts.push(line);
        } else {
          const line = renderPerson(entry, schemaV2, 1, 1, 1);
          if (line) parts.push(line);
        }
      }
      return capResult(parts.join(". "), limitChars);
    } catch {
      return "";
    }
  }
  ...
  return { ingest, extract, recall, recallCompact, getOrLoadProfile, bufferByScope, inflightExtract, profileCache, flush: flushScope, stop };
}
