...
export function createSocialMemory({ cfg, llm, stateDir, log }) {
  ...
  function flushScope(scope) {
    ...
    const profile = profileCache.get(scope);
    if (!profile) return;
    writeProfileFile(scope, profile);
  }

  function writeProfileFile(scope, profile) {
    const parsed = parseScope(scope);
    if (!parsed) return;
    const { dir, file } = scopeToPath(stateDir, parsed.agentId, parsed.sessionKey);
    ensureDir(dir);
    ...
    try {
      ...
      _log.warn(`social-memory: write error for ${scope}: ${err?.message || err}`);
  ...
  async function extract(scope) {
    try {
      ...
      _log.warn(`social-memory: extract error for ${scope}: ${err?.message || err}`);
    }
  }
  ...
  return { ingest, extract, recall, getOrLoadProfile, bufferByScope, inflightExtract, profileCache, flush: flushScope };
}
