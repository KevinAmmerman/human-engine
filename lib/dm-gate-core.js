 ...
 * Optional plan-619 fields: `"topicKey":"tk-<8+ hex/alnum>"` (durable topic
 * identity) and `"owner":"agent|user"` (agent-owed skip). Both absent on
 * legacy envelopes; present-but-invalid fails the envelope.
...
export function evaluateDmGate(candidate, ctx) {
  ...
  // Plan 619: durable topic-ledger gates. Only evaluated when the candidate
  // carries a topicKey (topic-less candidates stay fail-open / byte-identical).
  // `ctx.topicState` is injected by the caller from the shared initiative
  // ledger ({ attempts, cooldownUntil, openAttemptAt }); absent state fails
  // open so the CLI and pre-635 producers are unaffected.
  if (candidate?.topicKey) {
    const ts = ctx?.topicState || null;
    check("topic-cooldown", !(ts?.cooldownUntil > now));
    check("topic-attempts", !((ts?.attempts ?? 0) >= (dcfg.topicMaxAttempts ?? 3)));
    check("topic-open", !(ts?.openAttemptAt && now - ts.openAttemptAt < (dcfg.openAttemptCooldownMinutes ?? 240) * 60000));
    check("agent-owed", candidate.owner !== "agent");
  }
...
export function envelopeError(envelope) {
  ...
  // Plan 619: optional durable-topic fields (produced by plan 635). Absent =
  // valid (backward compatible); present-but-invalid fails the envelope so a
  // malformed topicKey/owner can never reach an ungated delivery decision.
  if (envelope.topicKey !== undefined) {
    if (typeof envelope.topicKey !== "string" || !/^tk-[a-z0-9]{8,}$/.test(envelope.topicKey)) return "bad-topic-key";
  }
  if (envelope.owner !== undefined) {
    if (envelope.owner !== "agent" && envelope.owner !== "user") return "bad-owner";
  }
  return null;
}
...
export function candidateFromEnvelope(envelope, draftText, sessionKey, agentId) {
  return {
    ...
    // Plan 619: optional durable-topic identity + ownership carry through to
    // the gate. Absent for legacy/pre-635 envelopes (gate checks fail-open).
    topicKey: typeof envelope.topicKey === "string" ? envelope.topicKey : undefined,
    owner: typeof envelope.owner === "string" ? envelope.owner : undefined,
    sessionKey,
    agentId,
  };
}
