...
export function typingMs(text, wpm) {
  const w = wpm || 40;
  const cps = w * 5 / 60;
  const base = ((text || "").length / cps) * 1000;
  const jitter = lognormalRaw(1, 0.25);
  return Math.round(clamp(base * jitter, 1200, 60000));
}
...
export function scheduleForBubbles(bubbles, ctx, timingCfg) {
  ...
  for (let i = 0; i < bubbles.length; i++) {
    if (i === 0) {
      offset += readingDelayMs(ctx, tc) + thinkPauseMs() + typingMs(bubbles[i].content, wpm);
    } else {
      offset += bubbleGapMs(tc) + typingMs(bubbles[i].content, wpm);
    }
