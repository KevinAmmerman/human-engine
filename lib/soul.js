...
export async function enhanceAndWrite(cfg, engine, agentId) {
  const soulPath = resolveSoulPath(cfg);
  let raw;
  try {
  ...
  const result = await engine.enhancePersona({ persona: personaToSend, agentId });
...
async function autoEnhance(cfg, engine, agentId) {
  const soulPath = resolveSoulPath(cfg);
  let raw;
  try {
    raw = fs.readFileSync(soulPath, "utf8");
...
export function maybeAutoEnhance(cfg, engine) {
  ...
  setTimeout(async () => {
    try {
      if (await autoEnhance(cfg, engine, undefined)) {
        try {
