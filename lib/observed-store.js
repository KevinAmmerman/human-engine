...
export function createObservedStore({ stateDir, log }) {
  ...
  function countLines(file) {
    try {
      ...
      if (err?.code !== "ENOENT") {
        _log.warn(`observed-store: count error for ${file}: ${err?.message || err}`);
      }
      return 0;
    }
  }
  ...
  function appendObserved(sessionKey, row = {}) {
    try {
      ...
      _log.warn(`observed-store: append error for ${sessionKey}: ${err?.message || err}`);
    }
  }
  ...
  function rotate(file) {
    try {
      ...
      _log.warn(`observed-store: rotation error for ${file}: ${err?.message || err}`);
    }
  }
    ...
    try {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          _log.warn(`observed-store: skipping corrupt line in ${sessionKey}: ${String(line).slice(0, 80)}`);
        }
      }
      return out.slice(-Math.max(1, last));
    } catch (err) {
      if (err?.code !== "ENOENT") {
        _log.warn(`observed-store: read error for ${sessionKey}: ${err?.message || err}`);
      }
      return [];
    }
  }
