...
export function createObservedStore({ stateDir, log }) {
  ...
  function countLines(file) {
    try {
      ...
      if (err?.code !== "ENOENT") {
        _log.warn(`human-engine: observed-store: count error for ${path.basename(file).replace(/\d{5,}/g, (m) => "…" + m.slice(-4))}: ${err?.message || err}`);
      }
      return 0;
    }
  }
  ...
  function appendObserved(sessionKey, row = {}) {
    try {
      ...
      _log.warn(`human-engine: observed-store: append error for ${redactSessionKey(sessionKey)}: ${err?.message || err}`);
    }
  }
  ...
  function rotate(file) {
    try {
      ...
      _log.warn(`human-engine: observed-store: rotation error for ${path.basename(file).replace(/\d{5,}/g, (m) => "…" + m.slice(-4))}: ${err?.message || err}`);
    }
  }
    ...
    try {
      ...
      for (const line of tail) {
        ...
        try {
          ...
          _log.warn(`human-engine: observed-store: skipping corrupt line in ${redactSessionKey(sessionKey)}: ${String(line).slice(0, 80)}`);
        }
      }
      ...
      if (err?.code !== "ENOENT") {
        _log.warn(`human-engine: observed-store: read error for ${redactSessionKey(sessionKey)}: ${err?.message || err}`);
      }
      return [];
    }
  }
