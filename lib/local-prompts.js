...
export function formatAge(ts, nowMs) {
...
function renderTranscriptLine(t, nowMs) {
  const age = formatAge(t?.ts, nowMs);
...
export function buildDecidePrompt({ transcript, persona, voiceCard, agentName, mediaKind, memoryContext, v2Contract }) {
  const systemLines = [
    ...
    "Message ages are shown like (vor 3h). If the newest message is hours or days old, you may acknowledge the late reply naturally — briefly, and never over-apologize; when in doubt, skip the acknowledgment.",
    v2Contract
  ...
  const transcriptBlock =
    transcriptLines.length > 0
      ? wrapUntrusted(transcriptLines.map((t) => renderTranscriptLine(t, nowMs)).join("\n"))
      : "(no recent messages)";
...
export function buildSplitPrompt({ draft, transcript, persona, voiceCard, styleConstraints, maxBubbles, replyTarget }) {
  ...
  const contextBlock =
    transcriptLines.length > 0
      ? LOG_START + "\n" + transcriptLines.map((t) => renderTranscriptLine(t, nowMs)).join("\n") + "\n" + LOG_END
      : "(none)";
  const userMessage =
    "Draft: " + (draft || "") +
    "\n\nContext:\n" +
