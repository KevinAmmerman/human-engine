...
export function createMood({ cfg, llm, stateDir, log, readTranscript }) {
  ...
  function onMessageReceived(event, ctx) {
    try {
      ...
      const isGroup = isGroupSessionKey(sk);
      if (isGroup && cfg?.mood?.groupsEnabled !== true) return;
      const key = keyFor(agentId, sk);
      const n = (counters.get(key) || 0) + 1;
      counters.set(key, n);
      if (counters.size > 4096) counters.clear();

      const refreshEvery = isGroup ? (cfg?.mood?.groupsRefreshEvery ?? 10) : (cfg?.mood?.refreshEvery ?? 5);
  ...
  function snapshotFor(agentId, sessionKey) {
    if (!sessionKey) return null;
    if (!isGroupSessionKey(sessionKey)) return null;
    if (cfg?.mood?.groupsEnabled !== true) return null;
    const state = applyDecay(readMood(stateDir, agentId, sessionKey), Date.now(), cfg?.mood?.decayHours);
    if (state.valence === 0 && state.energy === 0 && !state.note) return null;
    return { valence: state.valence, energy: state.energy };
  }

  return { onMessageReceived, onBeforePromptBuild, maybeUpdateMood, snapshotFor, counters };
}
