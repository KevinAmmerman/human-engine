...
export function createDmProactive({ cfg, llm, socialMemory, runtime, stateDir, log, now, activityFilePath }) {
  ...
  // Plan 536 (incident 2026-09-04, RC 1): the production message_sending ctx
  // carries NO sessionKey (OpenClaw 2026.8.1 PluginHookMessageContext =
  // {channelId, accountId?, conversationId?} — dist/plugin-sdk/src/plugins/
  // types.d.ts:2094; the three runMessageSending call-sites pass none). So
  // when ctx.sessionKey is absent or not a DM key, derive the target DM scope
  // from the EVENT (`event.to` + `ctx.channelId`/`event.metadata?.channel`).
  // Derivation ONLY proceeds when the target matches a known DM scope suffix
  // from the state (`agent:<agentId>:<channel>:direct:<to>`), or — as a
  ...
  // negative chat ids, `:group:` sessions) are never derived → fail-open.
  function deriveDmFromEvent(event, ctx) {
    ...
    // Group targets are never DMs → fail-open.
    if (/^-\d+$/.test(to)) return null;               // telegram negative chat id = group
    if (String(to).includes(":group:") || String(to).includes("@g.us")) return null;
    // (a) Match against a known DM scope suffix in the state:
    //   `agent:<agentId>:<channel>:direct:<to>` → take that agentId.
    for (const sc of Object.keys(budget)) {
      ...
      if (ch !== channel || kind !== "direct" || target !== to) continue;
      return { sk: skPart, agentId };
    }
    // (b) Single configured agent fallback (fresh state, no scope match yet).
    const agents = Array.isArray(cfg?.agents) ? cfg.agents : [];
    if (agents.length === 1) {
      return { sk: `agent:${agents[0]}:${channel}:direct:${to}`, agentId: agents[0] };
    }
    // (c) Ambiguous → warn + fail-open (never ratel a send).
