...
import { localDayKey, isQuietHour, overlapCount } from "./proactive.js";
 ...
 * never rise in reaction to Kevin's silence — this 48h rule is hard).
...
export function createDmProactive({ cfg, llm, socialMemory, runtime, stateDir, log, now, commitmentsPath }) {
  ...
  function saveBudget() {
    try {
      ...
      _log.warn(`dm-proactive: save error: ${err?.message || err}`);
    }
  }
  ...
  function appendLog(entry) {
    try {
      ...
      _log.warn(`dm-proactive: log append error: ${err?.message || err}`);
    }
  }
  ...
  // Inbound bookkeeping: a Kevin message after a care send counts as a reply
  ...
  async function onMessageSending(event, ctx) {
    try {
      ...
      // shadow: pass through, never rewrite or block.
      return;
    } catch {}
  }
  ...
  function reconcileCandidate(agentId, sk, content) {
    ...
    try {
      ...
      if (data && Array.isArray(data.commitments)) store = data.commitments;
    } catch {}
  ...
  async function render(candidate, memoryReference) {
    ...
    try {
      ...
      _log.warn(`dm-proactive: render error: ${err?.message || err}`);
  ...
  async function handleCandidate(candidate) {
    ...
    entry.gate = gate;
    if (!gate.pass) {
      _log.info(`human-engine: dm-proactive SHADOW id=${candidate.id} kind=${candidate.kind} reason=failed:${gate.reasons[0]}`);
      appendLog(entry);
    ...
    if (shadow) {
      _log.info(`human-engine: dm-proactive SHADOW id=${candidate.id} kind=${candidate.kind} reason=passed draft=${(rendered.draft || "").slice(0, 80)}`);
      appendLog(entry);
      return { log: entry, sent: false };
    }

    if (!runtime?.subagent?.run) {
      _log.warn(`human-engine: dm-proactive cannot send — api.runtime.subagent.run unavailable id=${candidate.id}`);
      entry.sent = false;
      appendLog(entry);
      return { log: entry, sent: false };
    }
    ...
    try {
      ...
      _log.info(`human-engine: dm-proactive SENT id=${candidate.id} kind=${candidate.kind} commitmentId=${candidate.id}`);
    } catch (err) {
      _log.warn(`human-engine: dm-proactive send failed id=${candidate.id}: ${err?.message || err}`);
    }
    appendLog(entry);
