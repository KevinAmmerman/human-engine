...
export function createDmProactive({ cfg, llm, socialMemory, runtime, stateDir, log, now, activityFilePath }) {
  ...
  function loadBudget() {
    try {
      ...
      // v3 (current): per-agent sentIds + byKind buckets, no migration needed.
      ...
      // bucket. scopes are kept unchanged. Legacy is read-only after this.
  ...
  function saveBudget() {
    try {
      ...
      const sentIdsOut = {};
      for (const [agentId, b] of Object.entries(sentBuckets)) sentIdsOut[agentId] = b.list;
      const byKindOut = {};
      for (const [agentId, bucket] of Object.entries(byKindBuckets)) byKindOut[agentId] = bucket;
      fs.writeFileSync(tmp, JSON.stringify({ version: STATE_VERSION, scopes: budget, sentIds: sentIdsOut, byKind: byKindOut }), { encoding: "utf8", mode: 0o600 });
  ...
  function deriveDmFromEvent(event, ctx) {
    ...
    const owners = [];
    for (const sc of Object.keys(budget)) {
