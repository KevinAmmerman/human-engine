...
import { isDmSessionKey, agentIdFromSessionKey, parseScope, isChatSession } from "./scope.js";
...
export function createDmProactive({ cfg, llm, socialMemory, runtime, stateDir, log, now, activityFilePath }) {
  ...
  function deriveDmFromEvent(event, ctx) {
    ...
    for (const sc of Object.keys(budget)) {
      ...
      const parsed = parseScope(skPart);
      if (!parsed || !parsed.agentId || parsed.channel !== channel || parsed.kind !== "direct" || parsed.rest !== uid || parsed.rest.includes(":")) continue;
      owners.push(parsed.agentId);
    }
