...
export function defaultConfig() {
  return {
    ...
    reactions: {
      hintEnabled: false, // opt-in: appendSystemContext-Fähigkeitshinweis (model-routed Reaktionen, nie plugin-seitig gesendet)
    },
    initiative: {
      enabled: false,       // master switch, default OFF (shadow-first, zero files/injection)
      shadow: true,         // shadow-log every candidate before any act
      agents: [],           // empty = all scoped agents
      scopes: ["group"],    // which session kinds participate
      everyMinutes: 60,
...
const NESTED_KEYS = ["selfVoice", "socialLearning", "socialMemory", "decide", "humanize", "timing", "naturalize", "proactive", "dmProactive", "mood", "threads", "reactions", "initiative"];
