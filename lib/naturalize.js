...
import { stripMetaCommentary, sanitizeTells, expandInlineLists, isFaithfulSplit } from "./anti-tell.js";
...
import { scheduleForBubbles } from "./timing-engine.js";
...
// P0 answer-delivery hotfix (HOTFIX #2): mechanical fragmentation fallback for
// the fact-guard. Zero LLM, zero rewriting — splits the draft into sentences
// and groups them greedily so every fact survives by construction. Never drops
// content: a bubble starts only when the current one already holds ≥1 sentence
// AND the next sentence would exceed 240 chars; remaining sentences beyond the
// hard maxBubbles cap merge into the LAST bubble (which may grow long).
export function fragmentDraft(text, maxBubbles = 5) {
  ...
  if (sentences.length <= 1) {
    return [{ content: raw, position: 0 }];
  }

  const cap = Math.max(1, Number.isFinite(maxBubbles) ? Math.floor(maxBubbles) : 5);
  const bubbles = [];
  let current = sentences[0];
  for (let i = 1; i < sentences.length; i++) {
    const next = sentences[i];
    const candidate = current + " " + next;
    const wouldOverflow = candidate.length > 240;
    if (wouldOverflow && bubbles.length + 1 < cap) {
  ...
  return bubbles.map((content, position) => ({ content, position }));
}
...
export function createNaturalize({ cfg, engine, persona, socialMemory, observedStore, mood, selfVoice, log, scheduleBubbles }) {
  const _log = log || { info() {}, warn() {}, debug() {} };
  const speakEpochTtlMs = cfg?.naturalize?.speakEpochTtlMs ?? SPEAK_EPOCH_TTL_MS;
  const _scheduleBubbles = scheduleBubbles || ((bubbles, ctx, timingCfg) => scheduleForBubbles(bubbles, ctx, timingCfg));
  ...
  async function flush(sk) {
    ...
    // Plan 615: fidelity guard. If the split LLM paraphrased/rewrote the draft
    // (pronoun flips, invented reactions/sentences) instead of splitting it,
    // fall back to the mechanical content-preserving fragmentation. Draft-only
    // (the transcript is never consulted), never runs on the raw-fallback path.
    const unfaithful =
      cfg?.humanize?.requireFaithfulSplit !== false &&
      engine &&
      scheduled.length > 0 &&
      !isFaithfulSplit(finalDraft, scheduled);
    if (unfaithful) {
      _log.warn(`human-engine: humanize unfaithful — mechanical split sk=${redactSessionKey(sk)}`);
    }
    ...
    // lost — mechanically fragment the draft into bubbles instead of delivering
    // one raw block. Draft-only (the transcript is never consulted), and
    // skipped on the raw-fallback path (respond returned null/superseded — no
    // bubbles to check). All facts survive by construction (no rewriting).
    let needsMechanical = unfaithful;
    if (!needsMechanical) {
      ...
      if (draftTokens.length >= 2) {
        ...
        if (missing.length > draftTokens.length / 2) {
          _log.warn(`human-engine: fact-guard fallback sk=${redactSessionKey(sk)} (${missing.length}/${draftTokens.length} facts missing from bubbles) — mechanical fragmentation (${fragmentDraft(finalDraft, cfg?.humanize?.maxBubbles ?? 5).length} bubbles)`);
          needsMechanical = true;
        }
      }
    }
    if (needsMechanical) {
      const maxBubbles = cfg?.humanize?.maxBubbles ?? 5;
      const fragments = fragmentDraft(finalDraft, maxBubbles);
      const timingCfg = cfg?.timing || {};
      const ctx = {
        isGroup,
        isQuestionReply: Boolean(replyTarget?.quotedName),
        wasAddressed: replyTarget != null,
        contentReadMs: 0,
        hourOfDay: new Date().getHours(),
        moodEnergy: (isGroup && mood?.snapshotFor ? mood.snapshotFor(agentIdFromSessionKey(sk) || null, sk)?.energy ?? null : null),
        newestAgeMs: null,
      };
      const fragmentsScheduled = _scheduleBubbles(fragments, ctx, timingCfg);
      cancelSessionTimers(sk);
      const fragTimers = [];
      const fragAbort = new AbortController();
      ...
      for (let i = 0; i < fragmentsScheduled.length; i++) {
        const frag = fragmentsScheduled[i];
        const timer = setTimeout(() => {
          void (async () => {
            if (fragAbort.signal.aborted) return;
            ...
            const fragReplyToId = i === 0 && replyTarget && replyTarget.replyToId ? replyTarget.replyToId : undefined;
            const ok = await deliverWithRetry(dispatcher, frag.content, ttsCtx, _log, fragReplyToId);
            if (ok === false) {
              _log.warn(`human-engine: FRAGMENT LOST sk=${redactSessionKey(sk)} part=${i + 1}/${fragmentsScheduled.length} — host aborted delivery`);
            } else {
              _log.info(`human-engine: fragment ${i + 1}/${fragmentsScheduled.length} sk=${redactSessionKey(sk)} sendBlockReply=${ok} len=${(frag.content || "").length}`);
            }
            if (i === fragmentsScheduled.length - 1) {
              cancelSessionTimers(sk);
        ...
        fragTimers.push(timer);
      }
      bubbleTimers.set(sk, fragTimers);
      return;
    }
