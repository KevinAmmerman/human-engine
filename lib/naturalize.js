...
import { isDmSessionKey, agentIdFromSessionKey, channelAndRestFromSessionKey } from "./scope.js";
...
export function createNaturalize({ cfg, engine, persona, socialMemory, observedStore, log }) {
  ...
  async function flush(sk) {
    ...
    const systemPrompt = persona?.buildPersonaPromptWithMemory?.(cfg, state, sk, agentIdFromSessionKey(sk)) || null;
