...
export function createNaturalize({ cfg, engine, persona, socialMemory, log }) {
  ...
  function onReplyDispatch(event, ctx) {
    ...
    try {
      ...
      // no explicit eviction needed. epoch is null here and bound at capture time
      // (reply_dispatch fires before the speak epoch is stashed by the gate).
      ...
      _log.info(`human-engine: dispatch armed sk=${sk} hasDispatcher=${Boolean(ctx.dispatcher)}`);
      return;
    } catch {
      return;
    }
  ...
  async function flush(sk) {
    ...
    if (cleaned.stripped) {
      _log.warn(`human-engine: meta-commentary stripped sk=${sk} before=${draft.length} after=${cleaned.text.length}`);
    }
    ...
    if (cleaned.commentary && !cleaned.stripped && finalDraft.length > 40 && engine?.regenerateReply) {
      ...
      if (regen?.text) {
        ...
        _log.warn(`human-engine: regenerated reply after pure-commentary sk=${sk}`);
      } else {
        _log.warn(`human-engine: REPLY SUPPRESSED sk=${sk} (pure commentary, regeneration failed)`);
