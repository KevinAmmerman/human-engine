...
// Plan 587: DM/group classification, exact copy of dm-proactive.js:48,54-58 so
// both paths agree on which sessions are direct chats (and thus never bubbled
// when naturalize.disableDM is on). Local helper per house convention.
...
function buildTtsContext(event, sk, ctx) {
  ...
  return {
    ...
    ttsAuto,
    channel,
    agentId: scopeFromSessionKey(sk) || ctx?.agentId || undefined,
...
export function createNaturalize({ cfg, engine, persona, socialMemory, observedStore, log }) {
  ...
  function onReplyPayloadSending(event, ctx) {
    try {
      ...
      pushTranscriptPeek(sk, "[" + (cfg.agentName || "Agent") + "] " + text.slice(0, 300), undefined, Date.now());
  ...
  async function flush(sk) {
    ...
    if (!respondResult || respondResult.superseded) {
      deliverRaw(sk, dispatcher, finalDraft, dstate, ttsCtx);
      return;
    }
    ...
    if (!scheduled || scheduled.length === 0) {
      await deliverRaw(sk, dispatcher, finalDraft, dstate, ttsCtx);
      return;
    }
    ...
    for (let i = 0; i < scheduled.length; i++) {
      ...
      const timer = setTimeout(() => {
        void (async () => {
          ...
          const ok = await deliverWithRetry(dispatcher, bubble.content, ttsCtx, _log);
          if (ok === false) {
  ...
  async function deliverRaw(sk, dispatcher, draft, dstate, ttsCtx) {
    const ok = await deliverWithRetry(dispatcher, draft, ttsCtx || null, _log);
    if (ok === false) {
