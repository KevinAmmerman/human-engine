...
export function saveCache() {
  try {
    ...
    fs.writeFileSync(tmp, JSON.stringify({ cache, counter }), "utf8");
...
export function createVoiceCard({ cfg, engine, stateDir: sd, log }) {
  ...
  setVoiceCardGetter((sk) => getCard(sk, cfg.socialLearning?.perSessionCard === true));
  ...
  function onBeforePromptBuild(event, ctx) {
    try {
      ...
      const perSession = cfg.socialLearning?.perSessionCard === true;
...
async function spawnRefresh(sessionKey, messages, cfgCtx, engine) {
  ...
  try {
    ...
    if (typeof promptBlock === "string" && promptBlock) {
      const perSession = cfgCtx.socialLearning?.perSessionCard === true;
